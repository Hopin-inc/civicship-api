...
`civicship.app` フロントエンドは **Next.js (App Router) で Google Cloud 配信** されている前提（`34.8.190.174` → `googleusercontent.com`）。DID Document も同 Next.js 内のルートとして実装する。

#### 5.4.1 新規: `app/.well-known/did.json/route.ts`
...
export async function GET() {
  const issuerJwk = await getKmsIssuerPublicJwk(); // §5.1.2 issuerDid.ts
  const document = {
    "@context": ["https://www.w3.org/ns/did/v1", "https://w3id.org/security/jwk/v1"],
    id: "did:web:civicship.app",
    verificationMethod: [{
      id: "did:web:civicship.app#key-1",
      type: "JsonWebKey2020",
      controller: "did:web:civicship.app",
      publicKeyJwk: issuerJwk,
    }],
    assertionMethod: ["did:web:civicship.app#key-1"],
    authentication: ["did:web:civicship.app#key-1"],
  };
  return NextResponse.json(document, {
    headers: {
      "Content-Type": "application/did+json",
      "Cache-Control": "public, max-age=300",  // 5 分（鍵ローテ即時反映優先）
      "Access-Control-Allow-Origin": "*",       // DID resolver は任意のオリジンから来る
    },
  });
}
```

- KMS 公開鍵を反映するので動的生成（メモリキャッシュ TTL 1h）
- リダイレクト禁止（W3C did:web 仕様）
- 静的化禁止（最新の Issuer 鍵を反映するため）

#### 5.4.2 新規: `app/users/[userId]/did.json/route.ts`

ユーザーごとの DID Document（`did:web:civicship.app:users:{userId}`）:
...
export async function GET(
  ...
  const anchor = await prisma.userDidAnchor.findFirst({
    where: { did: `did:web:civicship.app:users:${userId}`, status: "CONFIRMED" },
    orderBy: { createdAt: "desc" },
  });
  if (!anchor || anchor.operation === "DEACTIVATE") {
    return new NextResponse(null, { status: 404 });
  }
  const document = await reconstructDidDocument(userId, anchor);  // §5.1.4
  return NextResponse.json(document, {
    headers: {
      "Content-Type": "application/did+json",
...
- DEACTIVATE op が最新なら 404（DID 失効）

#### 5.4.3 新規: `app/anchor/[txHash]/verify/route.ts` (Optional)

任意の verifier が civicship 経由で chain 検証する場合のヘルパ:

- 内部で Blockfrost を叩いて metadata を返すのみ
- 必須ではない（verifier は直接 Cardano explorer / Blockfrost を叩けば良い）

#### 5.4.4 セキュリティ要件（civicship.app 全体）

これらのエンドポイントが信頼されるには civicship.app 自体の HTTPS 信頼が前提。§8.6 参照。
...
3. **CAA レコード設定** (Cloudflare Dashboard → DNS → CAA Record)
   ...
   civicship.app. CAA 0 iodef "mailto:security@civicship.app"
   ```
