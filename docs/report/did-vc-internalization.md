...
実装は `AnchorRecoveryService` として §5.2.3 に組み込む。
...
5. DB から User 行を削除（FK 関係上、UserDidAnchor / VcIssuanceRequest を先に手動削除 or 論理削除フラグ）
6. chain 上には DEACTIVATE 履歴と VC anchor の root が残るが、PII 不在
