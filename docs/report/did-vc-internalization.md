...
- `repository.ts`:
  - `TransactionAnchor` / `VcAnchor` / `UserDidAnchor` の Prisma クエリ
...
- `service.ts`:
  - `reserveNextIndex()`: VC 発行時に呼ばれる。trx 内で StatusListCredential.next_index を atomic CAS で増分し、index を返す。capacity 到達時は新 list を発行
  - `revokeIndex(listKey, index)`: 指定ビットを立てる。bitstring を更新し、新版 VC JWT を再発行（KMS 署名）
  - `getEncodedList(listKey)`: HTTPS 配信用に最新の VC JWT を返す
- `usecase.ts`:
...
#### 5.4.6 オプション: `/anchor/:txHash/verify`
...
[Day 1+]  全 resolver キャッシュが切れて 24h 経過
          KMS の N を rotating-out として deactive 化
          DID Document.verificationMethod = [#key-N+1]
          (旧鍵で署名された VC の検証は §9.1.4 で説明)
```

→ §5.4.3 の `IssuerDidService.buildDidDocument` で複数鍵を `verificationMethod` に並べる実装。

#### 9.1.3 Cardano wallet 鍵の HSM 保管と single-payment-key 設計
...
#### 9.1.5 将来 VP 対応が必要になった場合（§B の別ケース）
...
```
[User 削除リクエスト受領]
1. UserDidAnchor の DEACTIVATE op を発行（次回バッチで chain にも記録）
2. X 日経過（DEACTIVATE op が CONFIRMED になるのを待つ）
3. DB から User 行を削除（Cascade で UserDidAnchor 全行削除）
4. VC については revocation list に登録（StatusList ビット立て）
5. chain 上には DEACTIVATE 履歴が残るが、PII 不在（hash と DID 文字列のみ）
```

→ EU ユーザー対応時はこのフローを Phase 4 後の独立タスクで実装。本 PR では「`onDelete: Cascade` を `Restrict` に変更し、削除フローを別途設計する」と明記する。

→ 設計書 §4.1 の `User.onDelete: Cascade` を **`Restrict`** に変更（または `SetNull`）し、ユーザー削除は明示的な workflow を経るようにする。
...
9. VC の anchor も chain で確認したい場合（VC inclusion proof）:
   - VcAnchor を leafIds に該当 vcRequest.id を含むものを検索
   - VC JWT の Blake2b-256 hash を計算 → leaf hash
   - VcAnchor.leafIds から Merkle 木を再構築 → proof を生成
   - proof で root に到達 → metadata 1985 の vc.root と一致 → OK（§A 対応）
