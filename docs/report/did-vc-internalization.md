...
- ユーザー鍵 (Ed25519) を生成 → `did:web:api.civicship.app:users:{userId}` 形式で返す
...
Express router を 1 ファイルにまとめる。**認証 middleware は適用しない**（per-route 設計のため bypass は記述しないだけ。グローバル auth は元々ない）。

```ts
import express from "express";
import { container } from "tsyringe";
import { PrismaClient } from "@prisma/client";
import { IssuerDidService } from "@/application/domain/did/issuerDidService"; // §5.1.2 相当
import { UserDidDocumentService } from "@/application/domain/did/userDidDocumentService"; // §5.1.4
...
router.get("/.well-known/did.json", async (_req, res) => {
  ...
  res.set("Cache-Control", "public, max-age=300"); // 5 分（鍵ローテ即時反映）
  res.json(document);
});
...
// ... 既存コード ...
app.use(didRouter);  // パスプレフィックスなし: /.well-known/did.json と /users/:userId/did.json を生やす
```

注意:
- パスプレフィックスを付けない（W3C did:web 仕様で `/.well-known/did.json` は固定パス）
- 認証 middleware を**個別 route に適用しない**ことで認証 bypass を実現（Express 4 の per-route middleware パターン）
- リダイレクト禁止（W3C did:web 仕様）
- 静的キャッシュ禁止（KMS 鍵 / DID Document 更新を即時反映するため）

#### 5.4.3 IssuerDidService の責務（§5.1.2 補足）
...
export class IssuerDidService {
  ...
  async buildDidDocument(): Promise<DidDocument> {
    const jwk = await this.kms.getIssuerPublicJwk(); // メモリキャッシュ TTL 1h
    return {
...
#### 5.4.4 UserDidDocumentService の責務（§5.1.4 補足）
...
export class UserDidDocumentService {
  constructor(
    ...
    @inject("MerkleTreeBuilder") private merkle: MerkleTreeBuilder,
  ) {}

  async buildDidDocument(userId: string): Promise<DidDocument | null> {
    const did = `did:web:api.civicship.app:users:${userId}`;
    const anchor = await this.prisma.userDidAnchor.findFirst({
      where: { did, status: "CONFIRMED" },
      orderBy: { createdAt: "desc" },
    });
    if (!anchor) return null;
    if (anchor.operation === "DEACTIVATE") return null; // 404

    const baseDocument = decodeCbor(anchor.documentCbor);  // §3.3 で chain に格納した CBOR
    return {
      ...baseDocument,
      proof: {
        ...
        merkleRoot: anchor.documentHash,  // この user の最新 doc hash
        anchoredAt: anchor.confirmedAt?.toISOString(),
        verificationUrl: `https://cardanoscan.io/transaction/${anchor.chainTxHash}`,
      },
    };
  }
}
...
#### 5.4.5 オプション: `/anchor/:txHash/verify`

任意の verifier が civicship 経由で chain 検証したい場合のヘルパ。必須ではなく、verifier は直接 Cardano explorer / Blockfrost を叩けば良い。Phase 1 では未実装で OK。

#### 5.4.6 セキュリティ要件（civicship.app / api.civicship.app）

これらのエンドポイントが信頼されるには civicship.app（および api.civicship.app）の HTTPS 信頼が前提。§8.6 参照。
...
→ §9 のコスト試算は **backfill 月のみ別計上**（一回限り **~$1**、その後は通常運用 ~$1/月）。
...
- KMS 内の per-user 鍵の管理コスト（1000 ユーザー × 鍵 = KMS では現実的でない）
...
§10 Phase 3 で実施する **1000 ユーザー DID** の初回 backfill:
...
- 全ユーザーの DID Document を Next.js App Router で HTTPS 配信開始（`app/users/[userId]/did.json/route.ts`）
...
| ~~Q4~~ | ~~アンカリング粒度（1h vs 24h vs 件数閾値）~~ → **クローズ: 週次バッチ。Transaction Merkle root バックフィルは 1 tx に集約、DID backfill は ~13 tx（1000 ユーザー / 80 op/tx）に分割（§7.4 / §10 Phase 3 参照）** | ✅ クローズ |
...
6. https://api.civicship.app/users/u_xyz/did.json を HTTPS GET
...
検証で触れたインフラ:
  - api.civicship.app (HTTPS) ← 1 次解決のみ。倒産しても 7-9 で検証可能
