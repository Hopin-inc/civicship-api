...
> **Phase 1 実装状態**: 実装済 (PR #1099 → 後続 fix で `_signer` の自己 inject を削除済)。
> Phase 2 持ち越し項目は §16 の表を参照。
...
- リソース名 validation: `/cryptoKeyVersions/N` suffix を必須化、未指定の場合は明示 throw
- 署名出力は KMS が返す DER 形式から **raw 64 byte** に正規化して返す（CSL `Vkeywitness::new` / `did-jwt` 双方の interface に直接 inject 可能）
- 一時障害向け retry: 再帰実装の指数バックオフ（`UNAVAILABLE` / `DEADLINE_EXCEEDED` / 5xx で最大 3 attempts）

#### 5.1.2 新規: `src/infrastructure/libs/did/issuerDid.ts`

> **Phase 1 実装状態**: 実装済 (PR #1099 / IssuerDidService は PR #1100)。

- `did:web:api.civicship.app` の DID Document を返す
- KMS 公開鍵から **Multikey + `publicKeyMultibase`** を動的生成（`z` + base58btc(0xed01 + raw32) の Ed25519 multicodec）
- base58btc は手書き実装（ESM-only な `multiformats` を本体から排除し、test で参照ベクタとの等価検証を行う）
- メモリキャッシュ TTL 1h（公開鍵 bytes のみキャッシュ、Document 本体はリクエスト毎に組立）
- `keyVersionToFragment` は KMS リソース名末尾の `cryptoKeyVersions/N` から fragment を導出。suffix が無い場合は明示 `throw`（Gemini 指摘反映、silent fallback 廃止）
- 鍵ローテ時の挙動（旧鍵 `verificationMethod` 残し、新鍵を `assertionMethod` 先頭）は §G overlap window 仕様に依存。Phase 1 は **single-active-key** で運用、multi-key 配信は §16 の Phase 2 持ち越し
...
> **Phase 1 実装状態**: 実装済 (PR #1096)。RLS 取り扱いは PR #1104 で `internal()` に修正済（§5.4.x 参照）。

- ユーザー DID から DID Document を生成（DB の最新 UserDidAnchor から）
- proof フィールドに最新の anchor 情報を埋め込み（`PENDING` / `SUBMITTED` / `CONFIRMED` で proof block を切替、§F 参照）
- `CONFIRMED` 時は Blockfrost `getTransactionMetadata` 経由で metadata 1985 の存在を確認した上で `verificationUrl` を埋める
- `/users/:userId/did.json` から呼ばれる

#### 5.1.5 新規: `src/infrastructure/libs/blockfrost/client.ts`

> **Phase 1 実装状態**: 実装済 (PR #1096)。
...
- 主要メソッド:
  ...
  - `getTransactionMetadata(hash: string)` → `txsMetadata`
  - `awaitConfirmation(hash: string, timeoutMs: number)` → `txs` を polling
- リトライ・指数バックオフ内蔵
- `awaitConfirmation` の polling sleep は SonarLint `S7034` 対策で Promise executor を block 構文 `(resolve) => { setTimeout(resolve, ms); }` に統一（expression 構文を回避）

#### 5.1.6 新規: `src/infrastructure/libs/cardano/txBuilder.ts`

> **Phase 1 実装状態**: 実装済 (PR #1093)。tx 署名は raw seed (`CARDANO_PLATFORM_PRIVATE_KEY_HEX`) を採用。
> KMS 経由の vkey witness 手動 attach は §16 の Phase 2 持ち越し。

##### 概要
- `@emurgo/cardano-serialization-lib-nodejs` のラッパ
- `buildAnchorTx(input: { utxos, root, didOps, signKey, params }): SignedTx`
- `buildAuxiliaryData({ v, bid, ts, tx, vc, ops })` で metadata 1985 を組立 (§5.2.3 から呼出)
...
> **Phase 1 実装状態**: 実装済 (PR #1093)。

##### 概要
- `@openzeppelin/merkle-tree` ラッパ
- `buildRoot(leafIds: string[]): { root: Uint8Array; getProof(idx: number): Uint8Array[] }`
- leaf 並び替えは `(a, b) => a < b ? -1 : a > b ? 1 : 0`（**UTF-16 code-unit 比較**）で実装。`String.prototype.localeCompare` は locale / collation に依存して結果が変動するため**採用しない**。第三者 verifier が独立実装する際もこの比較規則を仕様化する
...
> **Phase 1 実装状態**: application 層実装済 (PR #1098 → GraphQL 配線 PR #1101 → Prisma 化 PR #1104)。
> 詳細な GraphQL Resolver 実装は §5.2.x、persistence 戦略の経緯は §17 を参照。
...
> **Phase 1 実装状態**: application 層実装済 (PR #1098 / #1101 / #1104)。
> `IssueVcInput` は `issuedAt?: Date` を受け、payload (JWT `nbf`) と persistence (`issuedAt`) で **同一 Date instance** を共有する。
> VC JWT の **KMS 署名は Phase 2 持ち越し**（現状は stub `STUB_SIGNATURE` を埋め込み、検証時に reject 想定）。§16 参照。
...
> **Phase 1 実装状態**: 実装済 (PR #1103)。
> Idempotency / Merkle root / metadata 1985 / 状態遷移は下記を参照。
> 署名は raw seed (`CARDANO_PLATFORM_PRIVATE_KEY_HEX`) で運用、KMS 経由 vkey witness 手動 attach は §16 の Phase 2 持ち越し。
> Cloud Run timeout は **600s** が必須（詳細は `docs/operations/anchor-batch-deploy-checklist.md`）。
> `runBatch` を fire-and-forget 化（Cloud Tasks 等への分離）も §16 の Phase 2 持ち越し。

`src/application/domain/anchor/`

実装上の確定事項:

- **Idempotency**: `getBatchTerminalStatus` で SUBMITTED / CONFIRMED / FAILED の terminal batch を早期 return、`claimPendingAnchors` で PENDING 行を CAS で本実行 batchId に固定
- **Merkle root の対象別 sort**:
  - `tx.root`: TransactionAnchor の leafIds (Transaction.id, ASCII byte 順)
  - `vc.root`: VcAnchor の vc_jwt 文字列 (ASCII byte 順)
  - `ops` 並び: UserDidAnchor.id (ASCII byte 順)
- **metadata 1985 構築**: `buildAuxiliaryData({ v, bid, ts, tx, vc, ops })` を txBuilder 側に集約
- **状態遷移**: PENDING → SUBMITTED → CONFIRMED / FAILED（FAILED は再試行で別 batch に取り込み）
...
> **Phase 1 実装状態**: 実装済 (PR #1102)。W3C Bitstring Status List 2021 準拠。
> StatusList VC JWT の署名は **Phase 2 持ち越し**（現状は stub `STUB_SIGNATURE_STATUS`、§16 参照）。

`src/application/domain/statusList/`

- `service.ts`:
  - `reserveNextIndex()`: VC 発行時に呼ばれる。trx 内で StatusListCredential.next_index を **atomic increment** で確保。
    - **frozen guard**: `frozen=true` の list は予約対象から除外
    - **bounded retry**: CAS 競合時は最大 5 attempt（reserve 系）/ 8 attempt（revoke 系）でバックオフ
    - **listKey 採番**: 新 list 作成時は `MAX(list_key::int) + 1` で次の整数を採番
  ...
  - `revokeIndex(listKey, index)`: 指定ビットを立てる。bitstring を更新し、新版 VC JWT を再発行（現状は stub 署名）
...
- `usecase.ts`:
  ...
  - **revoke 表現は `revokedAt` のみで完結**。`VcIssuanceStatus.REVOKED` enum 追加は schema PR の範囲外として §16 の deferred に積む
- `repository.ts`:
  - `StatusListCredential` の Prisma クエリ（atomic increment / frozen guard 含む）

公開 endpoint（§5.4.5）:

- `GET /credentials/status/:statusListId.jwt`
- CORS open（`Access-Control-Allow-Origin: *`）
- `Cache-Control: public, max-age=300`（5 分）

#### 5.2.5 GraphQL Resolver 配線

> **Phase 1 実装状態**: 実装済 (PR #1101)。

| 種別 | フィールド | 認可 |
|---|---|---|
| Query | `userDid` | 本人 + admin（usecase で `input.userId === permission.userId` を assert） |
| Query | `vcIssuance` | 本人 + admin |
| Query | `vcIssuancesByUser` | 本人 + admin |
| Mutation | `createUserDid` | `IsSelf` + usecase で input.userId === permission.userId assert |
| Mutation | `deactivateUserDid` | 同上 |
| Mutation | `issueVc` | `IsAdmin` |

- field resolver / DataLoader は最小実装に留め、関連オブジェクト解決は **§16 の Phase 1.5 持ち越し**（DataLoader 追加で N+1 解消）
- StatusList の **GraphQL revoke mutation** は §16 の Phase 1.5 持ち越し（現状は service / usecase 単体実装のみ）
...
> **Phase 1 実装状態**: `/.well-known/did.json` (PR #1100) と `/users/:userId/did.json` (PR #1097 → #1104) は実装済。
> `/credentials/status/:statusListId.jwt` (PR #1102) も実装済。
> `/vc/:vcId/inclusion-proof` は **暫定 501 Not Implemented**（PR #1097、batch worker 完成後に 200 化）。Merkle proof 生成は Phase 1.5 で予定。
...
> **Phase 1 実装状態**: 実装済 (PR #1100)。
> - **TTL 1h cache**（公開鍵 bytes のみキャッシュ）
> - active key 不在時は最小静的 Document を **200 で fallback** 配信（404 ではない）
> - §G overlap window 中の **multi-key Document 配信は §16 の Phase 2 持ち越し**。Phase 1 は single-active-key で運用
...
> **Phase 1 実装状態**: `/users/:userId/did.json` 経路で `DidDocumentResolver.buildDidDocument(userId)` を呼ぶ実装は PR #1097 で導入、PR #1104 で `internal()` に統一して RLS bypass を解消（旧実装は `public()` で取得できないケースが発生していた）。
> §9.2 の `USER_ID_REGEX` validation を入口で適用し、不正 userId は **400 早期返却**。
> `proof.state` の `PENDING` / `SUBMITTED` / `CONFIRMED` 切替は §F 参照。
...
既存の civicship-api CI/CD（lint → build → trivy → test matrix → aggregator、SonarCloud、CodeQL、Trivy daily 等）はすでに成熟している。本設計で追加する自動化は**最小限**に絞る。

### 15.1 設計方針

- **既存 `ci.yml` の test matrix を活用**: Phase 1 で実装する unit / integration test は `src/__tests__/{unit,integration,e2e,auth}/` に追加するだけ。新規 workflow ゼロ
- **新規 workflow は monitoring / canary 用途に限定**: 実装テストは既存に乗せ、新規は「**外部状態の継続監視**」のみ
- **doc と code の grep ベース整合チェックはやらない**: 設計書は **設計時の決定の捕獲** が役割。grep で同期を強制すると形式的な doc 更新で PR が増えるだけで実害ある drift は防げない。実装の source of truth は code、doc は判断履歴として腐るに任せる

### 15.2 既存 CI への統合（Phase 1 マージ時）

`src/__tests__/` 配下に普通に追加すれば、既存 ci.yml の test matrix（unit / integration / e2e / auth の 4 並列）が自動的に拾う:

| 場所 | テスト |
|---|---|
| `unit/` | `kmsSigner` (mock) / `userDidBuilder` / `merkleTreeBuilder` / `txBuilder` / `statusListService` / `userDidAnchorRepo` の version CAS |
| `integration/` | DID Document HTTPS 配信 / VC 発行〜anchor〜検証 / `/point/verify` の SQL / `/vc/:id/inclusion-proof` / GIN index の `EXPLAIN ANALYZE` |
| `e2e/` | Cardano preprod tx submit→confirm→verify の全周 / DID DEACTIVATE → VC cascade revocation |
| `auth/` | did router の per-route 認証 bypass が正しく適用 |

§12.4 性能 / §12.5 Chaos も同じ matrix に追加。**新規 workflow 不要、既存 ci.yml が拾う**。

### 15.3 新規 workflow 1: `tls-dns-posture.yml`

civicship.app の DNSSEC / CAA / HSTS が誰かの誤操作で外れた瞬間に検知する daily monitoring:

```yaml
name: TLS / DNS Posture (civicship.app)
on:
  schedule: [{ cron: '0 0 * * *' }]   # UTC 0:00 = JST 09:00
  workflow_dispatch:
...
jobs:
  check:
    ...
    steps:
      ...
      - name: DNSSEC check
        run: |
          dig +short DS civicship.app | grep -q . \
            || { echo "::error::DNSSEC DS record missing"; exit 1; }
      - name: CAA check
        run: |
          dig CAA civicship.app +short | grep -q 'pki.goog' \
            || { echo "::error::CAA pki.goog missing"; exit 1; }
      - name: HSTS preload header check
        run: |
          curl -sI https://civicship.app/ \
            | grep -i 'strict-transport-security' \
            | grep -q 'preload' \
            || { echo "::error::HSTS preload directive missing"; exit 1; }
      - name: Slack on failure
        if: failure()
        run: |
          curl -X POST ${{ secrets.SLACK_WEBHOOK_INFRA }} \
            -H 'Content-Type: application/json' \
            -d '{"text":":rotating_light: civicship.app TLS/DNS posture broken"}'
```

- 実装タイミング: **本 PR merge と並行で着手可能**（Phase 0 で DNSSEC / CAA を有効化したら翌日から走る）
- 既存 `trivy-daily.yml` と同じパターン（shielded runner、Slack 通知）

### 15.4 新規 workflow 2: `cardano-canary.yml`

Blockfrost / KMS / Cardano preprod の障害を毎朝検知する daily canary:

```yaml
name: Cardano Preprod Canary
on:
  schedule: [{ cron: '0 1 * * *' }]
permissions: { contents: read }
jobs:
  canary:
    ...
    steps:
      ...
      # Phase 0 PoC で書いた scripts/canary-anchor.ts を再利用
      - run: pnpm ts-node scripts/canary-anchor.ts
        env:
          BLOCKFROST_PROJECT_ID: ${{ secrets.BLOCKFROST_PREPROD }}
          KMS_KEY_VERSION: ${{ secrets.KMS_PREPROD_KEY_VERSION }}
      - if: failure()
        run: |
          curl -X POST ${{ secrets.SLACK_WEBHOOK_ENG }} \
            -d '{"text":":x: Cardano preprod canary failed"}'
```

- 実装タイミング: **Phase 0 PoC のスクリプトを再利用**して Phase 1 マージ後に有効化
- 動作: preprod に metadata 1985 で 1 件 submit → confirmation 待機 → metadata 取得して整合確認

### 15.5 既存 deploy workflow との統合

Phase 2 (shadow dual-write) / Phase 3 (cutover) で本番デプロイするとき、既存 `deploy-to-cloud-run-prd.yml` に feature flag 確認 step を追加（1-2 step の追加で済む、新規 workflow 不要）:

- `INTERNAL_DID_VC_ENABLED` の env 変数値が想定通りか
- KMS 鍵リング `civicship-issuer` に必要な鍵バージョンが存在するか

### 15.6 アプリ内 alert（運用フェーズ）

CI/CD ではなくアプリ実行時の話だが整理のためここに併記。既存の logger / Slack 通知基盤に乗せる:

| イベント | アラート条件 |
|---|---|
| Weekly batch 完了 | PENDING のまま 24h 経過 / FAILED → Slack |
| ADA 残高 | < 10 ADA → Slack（補充トリガー） |
| KMS API | 5xx 連続 3 回 → Slack |
| Blockfrost | 無料枠 80% 到達 → Slack（rate limit 警告） |
| anchor SUBMITTED | confirm に 1 時間以上 → Slack |

### 15.7 意図的にやらないこと

| 案 | 不採用理由 |
|---|---|
| 設計書 ↔ code の grep ベース整合チェック | doc は設計時の判断捕獲、grep 同期は形式コスト過剰、本物の drift は大規模 PR で同時更新される |
| Universal Resolver / Veramo の解決 canary | Phase 2 以降に必要が出てから |
| Cost tracking dashboard | 月額 $1 規模、必要が出てから |
| Cardano Hard Fork RSS watch | 年 1-2 回しか発生しない、手動 watch で十分 |
| KMS 鍵ローテ自動化 | 24h overlap 戦略が手動オペ前提（自動化は誤操作リスク高） |

---

## 16. Phase 2 / 1.5 持ち越し

Phase 1 の merge 済 (#1093 / #1094 / #1095 / #1096) ＋ open 中 (#1100 / #1101 / #1102 / #1103 / #1104) で確定した実装範囲を踏まえ、後続 phase に持ち越す項目を集約する。各項目は本文 §X からも個別に参照される。

| 項目 | Phase | 関連 PR |
|---|---|---|
| §G overlap multi-key 仕様（旧鍵を verificationMethod に並列配信） | Phase 2 | #1100 |
| KMS 経由署名（vkey witness の手動 attach、tx 署名を raw seed から KMS に切替） | Phase 2 | #1103 |
| `documentCbor` の chain inclusion（現状は documentHash のみ） | Phase 2 | #1103 |
| `runBatch` を fire-and-forget 化（Cloud Tasks 等への分離） | Phase 2 | #1103 |
| VC JWT の本番署名（現状 stub `STUB_SIGNATURE`） | Phase 2 | #1101 |
| StatusList VC JWT の本番署名（現状 stub `STUB_SIGNATURE_STATUS`） | Phase 2 | #1102 |
| DataLoader 追加（GraphQL Resolver field の関連オブジェクト解決） | Phase 1.5 | #1101 |
| GraphQL revoke mutation（StatusList の管理者向け operation） | Phase 1.5 | #1102 |
| `VcIssuanceStatus.REVOKED` enum 追加 | schema PR | - |
| `/vc/:vcId/inclusion-proof` の本実装（現状 501 Not Implemented） | Phase 1.5 | - |
| `@@index([userId, createdAt(sort: Desc)])` の追加 | Phase 1.5 | #1104 |

## 17. Phase 1 中の persistence 戦略（戦略 A）

Phase 1 中盤、schema PR が未 merge の期間中は **ローカル型宣言 + stub repository** の戦略 A を採用していた:

- application 層で `@prisma/client` の生成型を直接 import せず、ドメインローカルに `type UserDidAnchorRow = { ... }` を宣言
- repository は in-memory の Map を用いた stub 実装で振る舞いを担保
- service / usecase 層は型と振る舞いの両面で本番 Prisma 実装と互換になるよう設計

この戦略は PR #1104 で完全解消し、以下に統一済:

- `@prisma/client` の生成型 (`Prisma.UserDidAnchorGetPayload<...>` 等) を import
- repository は `PrismaClient` 経由の本番実装に置換
- domain ローカル型宣言は撤去

## 18. 運用要件（Phase 1 cutover の前提）

Phase 1 を本番に投入するために、インフラ側で事前に準備すべき項目:

| 項目 | 詳細 |
|---|---|
| **Cloud KMS Ed25519 鍵** | mainnet / staging の双方で鍵リング `civicship-issuer` を `global` location で生成（§9.1.5） |
| **Secret Manager: `BLOCKFROST_PROJECT_ID`** | mainnet / preprod の **両方**を登録 |
| **Cardano platform wallet** | mainnet で生成、**≥ 100 ADA** を funding（200+ 週分の anchor float） |
| **Secret Manager: `CARDANO_PLATFORM_PRIVATE_KEY_HEX`** | platform wallet の raw seed（KMS 移行は §16 の Phase 2 持ち越し） |
| **Cloud Run timeout = 600s** | anchor batch の confirmation polling を内包するため必須（`docs/operations/anchor-batch-deploy-checklist.md` 参照） |
| **Cloud Scheduler cron** | 週次 anchor batch をトリガー。認可は OIDC token + `CLOUD_SCHEDULER_TOKEN`（Secret Manager） |
| **DNSSEC + CAA レコード** | `civicship.app` で有効化（§9.6 の Phase 0 受け入れ基準） |
| **HSTS preload submission** | `api.civicship.app` を https://hstspreload.org/ に登録（civicship.app は登録済） |
