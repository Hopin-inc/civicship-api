...
### 1.1 目的
内製化の対象は **2 系統** あり、両方を同じ Blockfrost (Cardano) 経由のアンカリング基盤で扱う:

1. **Transaction の Merkle anchoring**（**本検討の発端**）
   - 2026-04-19 の `identus-cloud-agent-vm` ディスク枯渇で `/point/verify` が全 404 になった機能
   - 現状: civicship-api が `MerkleCommit` / `MerkleProof` をローカル計算し、IDENTUS Cloud Agent VM が Cardano に submit していた
   - 目標: civicship-api 自身が Blockfrost 経由で submit。週次バッチ
2. **DID/VC の発行**
   - 現状: IDENTUS API 経由で非同期ジョブ発行
   - 目標: `did:web` ＋ `did:key` ＋ `did-jwt-vc` で同等仕様を内製。Merkle anchoring は (1) と同じ基盤を流用

両系統共通:
- IDENTUS への依存を排除し、運用コスト・障害ポイント・ベンダーロックインを削減
- 暗号学的同値性を保ったまま、内部実装で同等以上の保証を提供
- 評価証跡を**改ざん不能な公開台帳に記録する事業要件**を、より安価かつ標準化された手段で達成

### 1.2 非ゴール
- 既存の DID 値・VC を「移行」して新方式に書き換えること（既発行は IDENTUS 形式のまま据え置く。新規分から内製方式）
- 既存 `t_merkle_commits` / `t_merkle_proofs` への新規書き込み・スキーマ拡張（**死蔵テーブル扱い**。新規 anchor は別テーブルで世代を切る）
- ユーザーへのウォレット配布・自己管理 DID（self-sovereign）への切替（あくまで platform-issued）
- 革新的な VC データモデル変更（既存 `EvaluationCredential` 構造はそのまま流用）

### 1.3 成功基準
- `/point/verify` が外部呼び出しなし（IDENTUS API 不使用）でローカル DB と Blockfrost 参照のみで応答
- 新規 DID/VC が IDENTUS API を一切呼ばずに発行される
- 第三者が公開台帳上で Transaction / 評価VC の存在を独立検証できる
- 月額運用コストが **$10/月以下** に収まる（現行費用比で大幅削減）
- 既存 GraphQL スキーマ（`DidIssuanceRequest` / `VcIssuanceRequest`）への破壊的変更ゼロ
- `/point/verify` のレスポンス形（`{ txId, status, transactionHash, rootHash, label }`）は維持
...
|---|---|---|---|
| **週次（7d）** ⭐ | ~4 | **~$0.30-0.40** | ~40 call/月（無料枠の 0.003%） |
| 日次（24h） | ~30 | ~$2-3 | ~300 call/月 |
...
- **バックフィル**: 5000 件全部を **1 tx に集約**（~$0.08）。「IDENTUS 時代以前の全レコードを内製アンカに移行」というスナップショットとして扱う
- **継続運用**: **週次バッチ**（**月 ~$0.30-0.40**）。粒度はあとから細かくできるが粗くは戻せないため、まず粗く始める
- 必要に応じてあとから日次・6h などに切り替え可能
...
// 新規テーブル: Transaction の Merkle anchoring（旧 t_merkle_commits / t_merkle_proofs は死蔵）
//
// 設計方針:
// - 旧 t_merkle_commits / t_merkle_proofs には FK も依存も持たない。完全に独立した世代として運用
// - leaf 集合は Postgres text[] (`leafIds`) に保持（5000 件 ≒ 125KB / row）
// - MerkleProof 相当のテーブルは作らない。proof は問い合わせ時に leafIds から動的生成
// - Cardano に直接 submit する状態を持つ（旧設計では IDENTUS が submit していた情報）
model TransactionAnchor {
  ...
  metadataLabel Int          @map("metadata_label")  // CIP-20 互換ラベル: 674
  chainTxHash   String?      @map("chain_tx_hash")   // submit 後に埋まる

  status        AnchorStatus @default(PENDING)       // Transaction では OPEN を使わない
...
- 既存 `t_merkle_commits` / `t_merkle_proofs` は **読み書きとも一切触らない**（DROP もしない、後日 audit が済んだら別マイグレーションで削除可）
...
| テーブル | 新方式での扱い |
|---|---|
| `t_merkle_commits` | **死蔵**。新規 INSERT 無し。SELECT も `/point/verify` 新実装からは行わない。IDENTUS 時代のレガシー証跡として保持 |
...
呼び出し元:
- `src/application/domain/transaction/verification/service.ts`（`TransactionVerificationService`）
- `src/application/provider.ts` で DI 登録

→ 用途は単一・限定的。新方式で**完全置換**して問題なし（Q1 はクローズ）。
...
```ts
// src/infrastructure/libs/point-verify/client.ts を以下で置換
@injectable()
  ...
  async verifyTransactions(txIds: string[]): Promise<VerifyResponse[]> {
    ...
    return txIds.map(txId => {
      ...
      return {
        ...
        transactionHash: a.chain_tx_hash ?? "",  // CONFIRMED 前は空
...
### 6.4 proof の動的生成（必要な場合）

公開検証 API として「proof も返す」要求が出た場合は、`leafIds` から木を再構築して該当 leaf の proof を生成:
...
async getProof(txId: string): Promise<{ root: string; siblings: string[]; chainTxHash: string }> {
  ...
  const leaves = anchor.leafIds.map(canonicalLeafHash);  // Transaction.id → 32B hash
...
5000 件の木構築は数 ms オーダー。検証側は Blockfrost で `metadataTxsLabel(674)` から root を独立取得し、proof と突き合わせるだけ。
...
| ~~Q1~~ | ~~`/point/verify` の現実の呼び出し元と使用頻度~~ → **確定: `TransactionVerificationService` 単独。Transaction Merkle 検証専用。新方式で完全置換可（§6 参照）** | クローズ | — |
...
| ~~Q4~~ | ~~アンカリング粒度（1h vs 24h vs 件数閾値）~~ → **確定: 週次バッチ。バックフィル分は 5000 件 → 1 tx に集約** | クローズ | — |
