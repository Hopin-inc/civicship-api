...
`@openzeppelin/merkle-tree` (JavaScript 実装) と同じ構築アルゴリズムを採用する:

1. リーフ集合 `L = [l_0, l_1, ..., l_{n-1}]` を以下のソート順で安定に並べる
   - `tx.root` の場合: **`transaction.id` の ASCII 昇順** (`ORDER BY id ASC`)
   - `vc.root` の場合: **VC JWT 文字列 (`vc_jwt`) の ASCII 昇順** (`ORDER BY vc_jwt ASC`)
   - 同一フィールドの完全重複は許容しない (二重 anchor 防止)
2. 各リーフを `leaf_hash` で 32 バイト hash に変換する
3. ライブラリ内部のツリー配列表現 (`tree.length = 2 * leaves.length - 1`) に従い、**奇数リーフでも複製は行わない** — 同層に対の存在しない葉は変更を加えず親ノードへそのまま昇格 (carry-up) させる
4. ペアが揃ったノードのみ `hash_pair(left, right)` で bottom-up に結合する
5. 最終的に得られた 32 バイト値が **Merkle root**

```
※ ペアの順序は左右の生成順を保持し、辞書順 sort は行わない
   (OpenZeppelin の SimpleMerkleTree 等で `sortLeaves: false`,
    sorted-pair なしに相当する設定を前提とする)。

例: 5 リーフのケース (carry-up; 複製なし)

           root = hash_pair(I01, H4)
          /                       \
        I01                       H4   ← 対の存在しない葉は
       /    \                          そのまま親階層へ昇格
      I0    I1
...
`@openzeppelin/merkle-tree` のデフォルト hash 関数は keccak256 だが、本仕様では Blake2b-256 を hash として注入する (同ライブラリは hash function injection をサポート)。なお、複製を採用する古典的 (Bitcoin 互換) Merkle 構築とは root が一致しないため、verifier は本仕様の carry-up 規則を実装する必要がある。
...
| `ops[].h` | hex string (64 chars) | yes (k!="d") | `ops[].doc` を結合して得られる **CBOR エンコード済みバイト列** (`Uint8Array`) を入力とする Blake2b-256 hash。すなわち `h = blake2b256( concat_bytes(ops[].doc[0], ops[].doc[1], ...) )`。JSON 文字列を再 hash しない (canonicalization 揺れ回避) |
| `ops[].doc` | array of bytes | optional | DID Document を **CBOR (RFC 8949) でエンコード** したバイト列。Cardano metadata の単一 bytes フィールド 64-byte 上限のため、CBOR エンコード後のバイト列を 64-byte 単位で連続スライスして配列化する (連結時はそのまま結合すれば元のバイト列が復元される) |
| `ops[].prev` | hex string (64 chars) | yes (k!="c") | 直前 op の Cardano tx hash (hash chain) |

### 5.2 サイズ制約と CBOR エンコード手順

Cardano metadata は単一 string / bytes フィールドが 64 バイト上限のため、以下の順序でエンコードする (検証時の再現性のため、必ずこの順序を守る):

1. DID Document (JSON-LD) を **CBOR (RFC 8949) でエンコード** しバイト列 `B` を得る
2. `h = blake2b256(B)` を 64 文字 hex で `ops[].h` に格納
3. `B` を 64-byte 単位で先頭から連続スライスし、`ops[].doc` 配列の各要素に格納する
   - 末尾要素は 64 バイト未満になり得る (パディングを行わない)
   - 復元側は `B = concat_bytes(ops[].doc[0], ops[].doc[1], ...)` で原バイト列を取り戻せる
4. CSL (`@emurgo/cardano-serialization-lib-nodejs`) の `TransactionMetadatum.new_bytes` / `new_list` で metadata 構造を組み立てる
...
| `docHash` | hex string (64 chars) | yes | `metadata.ops[opIndexInTx].h` と同一の値。すなわち DID Document を CBOR エンコードしたバイト列 (`ops[].doc` を連結したものと同じ) を入力とする Blake2b-256 hash。JSON 形式を再 hash しないこと |
...
```jsonc
{
  ...
  "leafIndex": 42,                    // 0-indexed (vc_jwt ASCII 昇順での位置)
  "siblings": [                       // bottom-up に並んだ sibling hash 列
    { "h": "<32-byte hex>", "side": "R" },
    { "h": "<32-byte hex>", "side": "L" }
    // carry-up 階層では sibling が存在せずこの配列に含まれない
  ],
...
verifier は `siblings` を順に `hash_pair` で適用し `root` を再構築、`chainTxHash` の metadata.vc.root と一致を確認する。`siblings[]` は §4.3 の carry-up 規則に従い構築されるため、対の存在しない階層は省略される (固定長 `log2(n)` ではない)。各 step で `siblings[i]` が provider 側で「左 sibling」「右 sibling」のどちらだったかを示すため、配列要素は `{ "h": "<hex>", "side": "L" | "R" }` の形式で返す。`side = "L"` のとき `current = hash_pair(sibling.h, current)`、`side = "R"` のとき `current = hash_pair(current, sibling.h)` と適用する。
