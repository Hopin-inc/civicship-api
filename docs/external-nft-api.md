...
| `POST` | `/api/nft-wallets/link` | ログイン時に vendor↔user の link を確立し walletRef を取得 |
| `POST` | `/api/nft-wallets/by-ref` | walletRef でウォレット登録 (server-side / webhook 用) |
...
`nft-tokens` / `nft-instances` 系エンドポイントが受け取る EVM アドレス
(コントラクトアドレス・`ownerWalletAddress`) は、Civicship 側で **すべて小文字に
正規化**して保存・検索する。チェックサム表記 (大小混在) で送っても小文字で
扱われるため、表記揺れによる重複や取得失敗は起きない。レスポンスでも小文字で返る。
...
| エンドポイント | 上限 |
| --- | --- |
| `PUT /api/nft-wallets/:walletAddress` / `POST /api/nft-wallets` / `POST /api/nft-wallets/link` | 1 リクエスト / 秒 / IP |
| `POST /api/nft-wallets/by-ref` | 120 リクエスト / 分 / IP (server-to-server webhook 用、バースト許容) |
| `PUT /api/nft-tokens/:address` | 10 リクエスト / 分 / IP |
| `PUT /api/nft-tokens/:address/instances/:instanceId` | 120 リクエスト / 分 / IP (mint バースト許容) |
...
| `403 Forbidden` | API キーに業者 (Vendor) 紐付けが無い、または対象の `NftToken` が他業者発行で書き換え不可 |
...
`PUT /api/nft-tokens/...` および `PUT /api/nft-tokens/.../instances/...` は **業者 (Vendor) 単位で認可**される。

- API キーは発行時に業者 (`BORDERLESS` / `KIBOTCHA`) に紐付けられる
- `PUT NftToken`: 既存レコードの `issuedByVendor` が **null (未 claim) または現業者と一致** の場合のみ書き込み可能。null の場合は最初の PUT で当該業者として claim される
- `PUT NftInstance`: 親 `NftToken.issuedByVendor` が **現業者と一致** の場合のみ書き込み可能 (claim されていない token への instance 登録は不可、先に token を PUT で claim する必要あり)
- 違反すると `403 Forbidden` を返す
...
## 1.5. server-side (webhook) からのウォレット登録

Shopify 購入 webhook など **エンドユーザーがブラウザにいない server-side 処理**から
ウォレットを登録するためのフロー。`POST /api/nft-wallets` (PUT 版含む) は Firebase ID
Token が必須で、Token の有効期限 (1 時間) の都合上 webhook まで持ち越せないため、
2 段階に分ける。
...
```
[ログイン時 / ブラウザあり]
  POST /api/nft-wallets/link
    → walletRef (不透明トークン) と既存ウォレット情報を取得
    → 業者は walletRef を自社 DB に保存

[購入 webhook / server-side]
  1. ウォレット W を生成
  2. POST /api/nft-wallets/by-ref { walletRef, walletAddress: W }
  3. レスポンスの created で分岐:
       created: true  → W に mint
       created: false → 返ってきた wallet.walletAddress (既存) に mint、W は破棄
```

ポイント: **mint は by-ref の後に行う**。Civicship が「正」と返したアドレスに mint
すれば、別プロジェクト経由で既にウォレットが登録済みでも取り違えが起きない。

### 1.5-a. POST /api/nft-wallets/link
...
ボディ不要。`(vendor, user)` の link を確立し、`walletRef` を返す。
...
| フィールド | 説明 |
| --- | --- |
| `walletRef` | webhook 登録に使う不透明トークン。業者 DB に保存する。Civicship 内部 ID は含まない |
| `wallet` | このユーザーの既存ウォレット。**未登録なら `null`** |

`wallet` が `null` でなければ既にウォレットがあるので、webhook では新規生成せず
その `walletAddress` に mint すればよい。

### 1.5-b. POST /api/nft-wallets/by-ref

```
POST /api/nft-wallets/by-ref
X-API-Key: <key>
Content-Type: application/json
```
...
| フィールド | 型 | 必須 | 説明 |
| --- | --- | --- | --- |
| `walletRef` | string | ✓ | `link` で取得したトークン |
| `walletAddress` | string | ✓ | 新規生成したウォレットアドレス |
| `name` | string |   | ユーザー名 (既存が「名前未設定」かつ新規作成時のみ反映) |
...
```json
{
  "success": true,
  "created": true,
  "wallet": { "walletAddress": "0xnew...456", "chain": "BASE_SEPOLIA" }
}
```

| フィールド | 説明 |
| --- | --- |
| `created` | `true` = 送った `walletAddress` で新規登録。`false` = 既存ウォレットがあったので**上書きせず**既存を返却 |
| `wallet.walletAddress` | mint すべき確定アドレス。`created: false` のときはここに mint する |

既存ウォレットは**絶対に上書きされない**。Firebase JWT は不要 (API キーのみ)。

### 主なエラー

| ステータス | 内容 |
| --- | --- |
| `400 Bad Request` | `walletRef` / `walletAddress` の形式不正 |
| `403 Forbidden` | API キーに vendor 紐付けが無い、または `walletRef` が他 vendor のもの |
| `404 Not Found` | `walletRef` が無効 (link が存在しない) |

### curl 例

```bash
# ログイン時
curl -X POST "${BASE_URL}/api/nft-wallets/link" \
  -H "X-API-Key: ${API_KEY}" \
  -H "Authorization: Bearer ${FIREBASE_ID_TOKEN}"

# webhook 時
curl -X POST "${BASE_URL}/api/nft-wallets/by-ref" \
  -H "X-API-Key: ${API_KEY}" \
  -H "Content-Type: application/json" \
  -d '{"walletRef":"3f9a...c1","walletAddress":"0xnew...456"}'
```
...
| `chain` | string | ✓ | チェーン環境。業者ごとに許可される値が異なる (BORDERLESS は `BASE_SEPOLIA`、KIBOTCHA は `POLYGON_MAINNET` / `POLYGON_AMOY`)。許可されない値は 400 |
| `name` | string |   | コントラクト名 |
| `symbol` | string |   | シンボル |
