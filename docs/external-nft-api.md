...
| `PUT` | `/api/nft-instances/:tokenAddress/:instanceId` | 単一 NFT インスタンスを登録/更新 |
...
`PUT /api/nft-tokens/...` および `PUT /api/nft-instances/...` は登録/更新用途のため、ユーザー JWT は不要 (API キーのみ)。
...
- `ownerWalletAddress` が Civicship に登録済みの `NftWallet` であること (`POST /api/nft-wallets` 経由で登録済み)
...
| フィールド | 型 | 必須 | 説明 |
| --- | --- | --- | --- |
| `ownerWalletAddress` | string | ✓ | 所有者の EOA アドレス。事前に `POST /api/nft-wallets` で登録済みであること |
| `name` | string |   | インスタンス名 |
...
- 所有権移転を取り込むには、移転先ウォレットを `POST /api/nft-wallets` で登録した上で `PUT /api/nft-instances/...` を新しい `ownerWalletAddress` で再送する。
