...
| `20260522062902_add_polygon_chains` | `NftChain` に `POLYGON_MAINNET` / `POLYGON_AMOY` 追加、EXTERNAL wallet の `chain` を NULL に戻す |
...
(EXTERNAL は EVM EOA で複数チェーン共通のため `chain = NULL`。
`20260522062902_add_polygon_chains` で EXTERNAL は NULL に揃え済み)。
