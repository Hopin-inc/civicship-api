...
describe("Membership Integration: Assign Owner Error Handling", () => {
  ...
  it("should fail to assign owner to non-existent user", async () => {
    ...
    await expect(
      membershipUseCase.ownerAssignOwner(
  ...
  it("should fail to assign owner in non-existent community", async () => {
  ...
  it("should fail when trying to assign owner role to user without membership", async () => {
    const user = await TestDataSourceHelper.createUser({
      name: "Test User",
      slug: "test-user",
      currentPrefecture: CurrentPrefecture.KAGAWA,
