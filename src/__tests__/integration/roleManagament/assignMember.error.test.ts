...
describe("Membership Integration: Assign Member Error Handling", () => {
  ...
  it("should fail to assign member to non-existent user", async () => {
    ...
    await expect(
      membershipUseCase.managerAssignMember(
  ...
  it("should fail to assign member in non-existent community", async () => {
  ...
  it("should fail when trying to assign member role to user without membership", async () => {
    const user = await TestDataSourceHelper.createUser({
      name: "Test User",
      slug: "test-user",
      currentPrefecture: CurrentPrefecture.KAGAWA,
