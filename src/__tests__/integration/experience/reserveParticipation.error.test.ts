...
describe("Reservation UseCase Business Logic Error Tests", () => {
  ...
  it("should fail to reserve with non-existent opportunity slot", async () => {
    ...
    ).rejects.toThrow(/not found|slot.*not.*found/i);
  });
  ...
  it("should fail to reserve with zero participants", async () => {
    ...
    await expect(
      useCase.userReserveParticipation({ input }, ctx)
    ).rejects.toThrow(/participant.*count|invalid.*count/i);
  });

  it("should fail to reserve with negative participants", async () => {
    const user = await TestDataSourceHelper.createUser({
      name: "Test User",
      slug: "test-user",
      currentPrefecture: CurrentPrefecture.KAGAWA,
