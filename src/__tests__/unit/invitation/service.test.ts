...
describe("OpportunityInvitationService", () => {
  ...
  describe("createOpportunityInvitation", () => {
    it("should create an opportunity invitation", async () => {
      ...
      const mockCreateInput: Prisma.OpportunityInvitationCreateInput = {
        code: "code123",
        opportunity: { connect: { id: "1" } },
      ...
      (OpportunityInvitationConverter.create as jest.Mock).mockReturnValue(mockCreateInput);
      ...
      expect(OpportunityInvitationRepository.create).toHaveBeenCalledWith(mockCreateInput, tx);
      expect(result).toEqual(mockResult);
    });
  });
