  ...
  async (req, res) => {
    try {
      ...
      await issuer.public(ctx, async (tx) => {
        walletRecord = await nftWalletService.createOrUpdateWalletAddress(
          ctx,
          user.id,
          walletAddress,
          tx,
      ...
      if (walletRecord) {
        issuer
          .internal(async (tx) => {
            ...
            if (result.success) {
              logger.info(
                `NFT metadata processed for wallet ${walletAddress}: ${result.itemsProcessed} items`,
              );
            } else {
              logger.warn(
                `NFT metadata processing failed for wallet ${walletAddress}: ${result.error}`,
              );
            }
          })
          .catch((error) => {
