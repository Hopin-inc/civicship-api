  ...
  async getJSON<T>(url: string, cfg?: AxiosRequestConfig): Promise<T> {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        throw new StripeApiError(res.status, res.data, url);
      } catch (error: unknown) {
        lastError = error;
        if (
          attempt < maxRetries &&
        ...
        if (isAxiosError(error) && error.response) {
          throw new StripeApiError(error.response.status ?? 0, error.response.data, url);
        }
        throw error;
      }
    }
...
export const createStripeHttpClient = (): AxiosInstance => {
  ...
  client.interceptors.request.use(
    (config) => {
      ...
      if (config.url?.includes("payment_intents") || config.url?.includes("customers")) {
  ...
  client.interceptors.response.use(
    (response) => {
      ...
      if (data && data.error) {
        throw new StripeApiError(response.status, data, response.config.url || "unknown");
      }
      return response;
    },
