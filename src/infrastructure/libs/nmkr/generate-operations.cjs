...
const outPath = path.resolve(__dirname, 'types.operations.d.ts');

if (!fs.existsSync(schemaPath)) {
  console.error(`[NMKR] ${schemaPath} がありません。先に 'pnpm nmkr:fetch:mainnet' を実行してください。`);
  process.exit(1);
}
...
function tsNameFromOperation(opId, method, url) {
  ...
  const hash = crypto.createHash('md5').update(method + url).digest('hex').substring(0, 6);
  ...
  for (const method of methodKeys) {
    ...
    if (requestBody) {
      ...
      if (contentInfo) {
        ...
        addType(`${baseName}RequestBody`, expr, `${method.toUpperCase()} ${url} request body${contentNote}`);
      }
    }
    ...
    for (const [code, response] of Object.entries(responses)) {
      ...
      if (content) {
        ...
        if (contentInfo) {
          ...
          const contentNote = contentInfo.type !== 'application/json' ? ` (${contentInfo.type})` : '';
          addType(`${baseName}Response${code}`, expr, `${method.toUpperCase()} ${url} ${code} response${contentNote}`);
        }
      }
    }
    ...
    if (successCodes.length > 0) {
      if (successCodes.length === 1) {
        addType(`${baseName}Response`, `${baseName}Response${successCodes[0]}`, `${method.toUpperCase()} ${url} success response`);
      } else {
        const union = successCodes.map(code => `${baseName}Response${code}`).join(' | ');
        addType(`${baseName}Response`, union, `${method.toUpperCase()} ${url} success response union`);
      }
    }
    ...
    if (errorCodes.length > 0) {
      if (errorCodes.length === 1) {
        addType(`${baseName}Error`, `${baseName}Response${errorCodes[0]}`, `${method.toUpperCase()} ${url} error response`);
      } else {
