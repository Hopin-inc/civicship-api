...
| 指標 | 値 |
|---|---|
| アクティブメンバー | [community_context.active_users_in_window]名 / [community_context.total_members]名（[active_rate_pct]%） |
...
| 総流通 [community_context.point_name] | [aggregate.points_sum] pt |

活動の内訳

| 区分 | 件数 | 流通 [community_context.point_name] |
|---|---|---|
| 感謝の贈り合い | [aggregates_by_reason.DONATION.tx_count]件 | [aggregates_by_reason.DONATION.points_sum] pt |
| 活動認定 | [aggregates_by_reason.GRANT.tx_count]件 | [aggregates_by_reason.GRANT.points_sum] pt |
| 新規参加の歓迎 | [aggregates_by_reason.ONBOARDING.tx_count]件 | [aggregates_by_reason.ONBOARDING.points_sum] pt |

※ aggregates_by_reason に該当 reason が存在しない場合はその行を省略する。
...
### [top_users[i].name]（受け取り：[top_users[i].points_in] pt / 贈った：[top_users[i].points_out] pt）
{{この人が何をした人かを highlight_comments から読み取って1文で。
読み取れない場合は「今週 [top_users[i].points_out] pt を贈りました。」等、数値の事実のみ書く。推測で埋めない。}}
}}
...
| 期間内活動参加者 | [community_context.active_users_in_window]名（参加率 [active_rate_pct]%） |
| 総取引件数 | [aggregate.tx_count]件 |
| 総流通量 | [aggregate.points_sum] pt |
...
- [highlight_comments[i].date]：「[highlight_comments[i].comment]」（[highlight_comments[i].points] pt / {{助け合いの種類を一言}}）
}}
