...
// Seeds always operate on the v1 row of each (variant, kind, scope)
// triple. Once a v2 (or later) is created via the admin UI / a future
// PR-F3 migration, those higher versions are managed there, not by
// this seed — pinning the seed to v1 keeps it from accidentally
// rewriting an experiment-in-progress.
...
export async function seedReportTemplates() {
  await prismaClient.$transaction(async (tx) => {
    for (const tmpl of TEMPLATES) {
      ...
      // Look up by (variant, communityId, kind, version) so:
      //   1. A JUDGE row and a GENERATION row at the same variant do
      //      not collide on the pre-F7 (variant, communityId) key.
      //   2. With multiple versions coexisting (PR-F3), the seed is
      //      deterministic — without an explicit version filter the
      //      lookup could pick whichever row Postgres returned first,
      //      potentially overwriting an inactive historical row while
      //      leaving the production v2 untouched.
      const existing = await tx.reportTemplate.findFirst({
        where: { variant: tmpl.variant, communityId: null, kind, version: SEED_VERSION },
        select: { id: true },
      });
      ...
      if (existing) {
        ...
        console.info(`  Updated SYSTEM template: ${tmpl.variant} (${kind})`);
      } else {
        ...
        console.info(`  Created SYSTEM template: ${tmpl.variant} (${kind})`);
      }
    }
  });
}
