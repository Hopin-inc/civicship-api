...
export async function seedReportTemplates() {
  await prismaClient.$transaction(async (tx) => {
    for (const tmpl of TEMPLATES) {
      ...
      // Look up by (variant, communityId, kind) so a JUDGE row and a
      // GENERATION row at the same variant do not collide on the
      // pre-F7 (variant, communityId) key.
      const existing = await tx.reportTemplate.findFirst({
        where: { variant: tmpl.variant, communityId: null, kind },
        select: { id: true },
      });
