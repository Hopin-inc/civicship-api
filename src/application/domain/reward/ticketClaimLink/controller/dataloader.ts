...
async function batchTicketClaimLinksById(
  ...
  const map = new Map(records.map((record) => [record.id, TicketClaimLinkPresenter.get(record)])) as Map<string, GqlTicketClaimLink | null>;

  return claimLinkIds.map((id) => map.get(id) ?? null);
}

export function createTicketClaimLinkLoader(issuer: PrismaClientIssuer) {
  return new DataLoader<string, GqlTicketClaimLink | null>((keys) => batchTicketClaimLinksById(issuer, keys));
}
