...
import * as Conv from "./data/converter";
...
export default class NftMintService {
  ...
  async mintNow(
    ...
    const mint = await tx.nftMint.findUnique({ where: { id: mintId }, ...T.nftMintSelectBase });
  ...
  markMinted(ctx: IContext, tx: Prisma.TransactionClient, id: string, txHash: string) {
    return this.repo.update(ctx, id, Conv.buildMarkMinted({ txHash }), tx);
  }

  markFailed(ctx: IContext, tx: Prisma.TransactionClient, id: string, error: string) {
    return this.repo.update(ctx, id, Conv.buildMarkFailed({ error }), tx);
  }
}
