...
async function testWorkingEndpoints() {
  ...
  try {
    ...
    await testEndpoint('GetPaymentAddressForRandomNftSale', () => {
      // Use global IP instead of localhost to avoid 406 errors
      const testIp = process.env.NMKR_TEST_IP || '8.8.8.8';
      return nmkrClient.getPaymentAddressForRandomNftSale(projectUid, 1, testIp);
    });
    await testEndpoint('GetAllAssetsInWallet', () => {
      // Use 60s timeout for wallet asset enumeration (preprod can be slow)
      return nmkrClient.getAllAssetsInWallet(testWallet, { timeoutMs: 60000 });
    });
